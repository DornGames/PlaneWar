...
const int   BULLET_DMG_MAX = 8;            // 基础攻击上限（3基础 + 5永久）

/*----------------------- 生命/防御与装备机制常量 -----------------------*/
const int   PLAYER_BASE_HP = 100;          // 玩家基础血量
...
inline int enemyArmor(EnemyType t, int level) {
    ...
    int a = base + (level - 1) / 4;
    const int ARMOR_CAP = 8;
...
inline int enemyHp(EnemyType t, int level) {
    ...
    int n = level - 1;
    int v = (int)(base * (1.0 + 0.30 * n + 0.018 * n * n));
    return v < 1 ? 1 : v;
}
...
inline int bossHp(int level) {
    // 【数值框架】Boss 血量 = 160 × (1 + 150%·n + 9%·n²)
    // 依据：玩家 DPS 全程增长约 60×（攻击 9× × 火力 3× × 射速 2.2×），
    // 故 Boss 血量必须同量级增长，才能维持 10~20 秒的击杀节奏。
    int n = level - 1;
    return (int)(160 * (1.0 + 1.50 * n + 0.090 * n * n));
}
...
inline int bossArmor(int kind, int level) {
    // 【数值框架】Boss 护甲 = 1 + n/6，上限 6（不同种类微调）。
    // 上限刻意低于杂兵，因为 Boss 已靠海量血量提供压力，护甲过高会拖长战斗。
    int bonus = (kind == BK_DREADNOUGHT) ? 1 : 0;      // 无畏舰重装 +1
    int a = 1 + bonus + (level - 1) / 6;
    const int BOSS_ARMOR_CAP = 6;
...
inline int enemyTouchDmg(EnemyType t, int level) {
    // 【数值框架】伤害 = 基数 × (1 + 12%·n + 0.5%·n²)
    int base;
    switch (t) {
    case E_FAST:    base = 14; break;
    case E_NORMAL:  base = 18; break;
    case E_ZIGZAG:  base = 18; break;
    case E_SHOOTER: base = 20; break;
    case E_STRAFER: base = 20; break;
    case E_TANK:    base = 26; break;   // 重装撞击
    case E_DIVER:   base = 32; break;   // 自爆俯冲最痛
    default:        base = 18; break;
    }
    int n = level - 1;
    return (int)(base * (1.0 + 0.12 * n + 0.005 * n * n));
}
...
inline int enemyBulletDmg(int level) {
    int n = level - 1;
    return (int)(HIT_HP_BULLET * (1.0 + 0.12 * n + 0.005 * n * n));
}
